...
-- Only the Json Data column ($2 in the source CSV) is landed; id and
-- updatetimestamp are generated by the table defaults (AUTOINCREMENT /
-- CURRENT_TIMESTAMP), so they're not listed in the COPY column list.
CREATE PIPE IF NOT EXISTS invoice_ocr_pipe
    AUTO_INGEST = TRUE
AS
    COPY INTO raw_invoice_ocr (json_data)
    FROM (
        SELECT $2
